...
def calculateVectorDiagramData(self):
    ...
    try:
        ...
        maxFrame=maxLoad=0
        for i,frame in self.finalKeyFrameStructures.items():
            loads=frame['loads']
            maxToad=max(*[x['peak'] for x in loads])    
            if maxToad > maxLoad : maxFrame=i;maxLoad=maxToad
        calc= lambda x: function(convertTo3D(x),masterData[maxFrame],self.shearMode,self.inextensibleMode,False,self.NoOfPointsInVector,1)
        rescaler=[calc(x)[component] if component else calc(x) for x in segments]  
        zoomFactor,_=getRescaledData([convertTo3D(x) for x in segments],rescaler)

        self.ZoomFactorResponse,self.ZoomFactorAction=zoomFactor,zoomFactor
        if self.vectorDiagramOptions.currentIndex()>3:            
            parameters=[component,self.shearMode,self.inextensibleMode,False,self.NoOfPointsInVector,self.ZoomFactorResponse]  
        else:
            parameters=[component,self.shearMode,self.inextensibleMode,False,self.NoOfPointsInVector,self.ZoomFactorAction]  
...
def calculateFrameMotionData(self):
    ...
    for i,frame in self.finalKeyFrameStructures.items():
        ...
        maxToad=max(*[x['peak'] for x in loads])    
        if maxToad > maxLoad : maxFrame=i;maxLoad=maxToad
    segments=list(self.df[(self.df['Flag']==True)&(self.df['Class']=='segment')]['Robject'])
    calc= function(self.finalKeyFrameStructures[maxFrame],masterData[maxFrame],shear,inextensible,self.NoOfPointsInMotion,1)
    # rescaler=[calc(x)[component] if component else calc(x) for x in segments]  
    zoomFactor,_=getRescaledData([convertTo3D(x) for x in segments],makeFunData(calc))
    self.ZoomFactorMotion=zoomFactor
    for i in keyFrames:

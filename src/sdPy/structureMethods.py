...
from sdPy.extensions import childOf, simplify
from sdPy.functionDefinitions import apply, dictInArr, rowPos, tolClose, unit
from sdPy.loadMethods import lineGlobalFEA
# from sdPy.numbaFunctions import lineTempGlobalFEA
from sdPy.segmentMethods import (lineStiffnessGlobal, lineStiffnessGlobal2,
...
from sdPy.supportMethods import rxn

def sortXY(mat):
    temp = unique(mat,axis=0)
    xs = unique(temp[:,0])
    ys = array([temp[:,1][t] for t in [(temp[:,0]==xs[q]).nonzero()[0] for q in range(xs.shape[0])]])
    sortSign = array([(-1)**x for x in range(xs.shape[0])])
    Ys = []
    [Ys.extend(sortSign[t]*sort(sortSign[t]*ys[t])) for t in range(xs.shape[0])]
    temp[:,1]=Ys
    return temp
...
def frame3d(struct,shear=False,inextensible=True,simplified=True):
    ...
    filter1 = lambda x: apply(lambda t: tolClose(t['location'],x['P1']), supports)
    filter2 = lambda x: apply(lambda t: tolClose(t['location'],x['P3']), supports)
    
    suspects = supports[apply(lambda x: x['type']=='Internal Hinge', supports)]
    def hinged(seg):
        if(suspects.size==0):
            return False
        else:
            hinged = True if any(apply(lambda x: tolClose(x['location'],seg['P3']), suspects)) else False
            return hinged
    ...
    def z1(seg):
        ...
        sps = sps[apply(lambda x: x['type']!='Node',sps)] if sps.size!= 0 else sps
        normals = apply(lambda x: x['normal'],sps) if sps.size!=0 else array([ array([0,0,1]) ])
    ...
    start = apply(lambda x: rowPos(x,nodes), apply(lambda x: x['P1'],segments))
    stops = apply(lambda x: rowPos(x,nodes), apply(lambda x: x['P3'],segments))
    ...
    jforce = loads[apply(lambda x: x['degree']==-1 and type(rowPos(x['P1'],nodes))!=type(False), loads)]
    jmomnt = loads[apply(lambda x: x['degree']==-2 and type(rowPos(x['P1'],nodes))!=type(False), loads)]
    ...
    memLoadMatrix = zeros(12*shape(memberLocation)[0]).reshape(shape(memberLocation)[0],12)
    jlods = hstack((jforce,jmomnt))
    memloads = loads[apply(lambda x: x['degree']>=0 or type(rowPos(x['P1'],nodes))==type(False) ,loads)]
    ...
    for x in memloads:
        memLoadMatrix[dictInArr(x['parentSegment'],segments)] += lineGlobalFEA(x['parentSegment']['P1'],
            x['parentSegment']['P3'],x['P1'],x['P3'],z1(x),z2(x),x['degree'],x['peak'],
    ...
    if unique(settlements).any():
        def add(x):
            K = lineStiffnessLocal(x['youngsModulus'],x['Iyy'],x['Izz'],norm(x['P3']-x['P1']),x['area'],x['shearModulus'],x['J'],shear,inextensible,hinged(x))
            T = lineTransformation(x['P1'],x['P3'],x['axisVector'],z1(x),z2(x))
            settlement1 = sum(apply(lambda t: t['settlement'],supports[filter1(x)]),0) if filter1(x).any() else zeros(6)
            settlement2 = sum(apply(lambda t: t['settlement'],supports[filter2(x)]),0) if filter2(x).any() else zeros(6)
            settlements = hstack((settlement1,settlement2))
            FEAs = T.T @ (K @ (T @ settlements)) if settlements.any() else zeros(12)
            return FEAs
    ...
    for id in arange(segments.size):
        ...
        vec = array([locationMatrix[nj],locationMatrix[nk]]).astype(int).flatten()
        sel = (vec>0).nonzero()[0]
        superStiffness[repeat(vec[sel]-1,sel.size),tile(vec[sel]-1,sel.size)] += lineStiffnessGlobal(x['P1'],x['P3'],x['axisVector'],z1(x),z2(x),x['youngsModulus'],x['Iyy'],x['Izz'],norm(x['P3']-x['P1']),x['area'],x['shearModulus'],x['J'],shear,inextensible,hinged(x))[repeat(sel,sel.size),tile(sel,sel.size)]
    ...
    deflections = matmul(inv(superStiffness),superJload-superFEA) if superStiffness.size!=0 else 0

    locvec = locationMatrix.flatten().astype(int)
    
    responses = apply(lambda x: deflections[x-1] if x!=0 else 0, locvec).reshape((shape(nodes)[0],6))
    ...
    responses = responsesRaw[apply(lambda x: type(rowPos(x,nodesOrg))!=type(False),nodes)]
    ...
    for id in arange(segments.size):
        ...
        globalDeflections = apply(lambda x: deflections[x-1] if x!=0 else x, vec)
        memLoad = lineTransformation(x['P1'],x['P3'],x['axisVector'],z1(x),z2(x)) @ memLoadMatrix[id] 
        localDeflections = matmul(lineTransformation(x['P1'],x['P3'],x['axisVector'],z1(x),z2(x)),globalDeflections)
    ...
    for i in arange(segmentsOrg.size):
        for x in childOf(segmentsOrg[i],structure):
            ...
            if x['parent'] == None:
                actions[2*i],actions[2*i+1]=actionsRaw[2*id],actionsRaw[2*id+1]
            else:
    ...
    globalActions = lambda index: multiplier*(lineTransformation(segments[index]['P1'],segments[index]['P3'],segments[index]['axisVector'],z1(segments[index]),z2(segments[index])).T @ hstack(actionsRaw[(2*index):(2*index+2),3:])).reshape((2,6))
    reactionsRaw = hstack((actionsRaw[:,:3],vstack(apply(globalActions,arange(segments.size)))))
    reactions = reactionsRaw[apply(lambda x: type(rowPos(x[0:3],nodesOrg))!=type(False),reactionsRaw)]

    addAllRxns = lambda node: sum(reactions[apply(lambda x: tolClose(node,x),reactions[:,:3]),3:],0)
...
def frame2d(struct,shear=False,inextensible=True,simplified=True,simFac = 0.9995):
    ...
    for x in jforce:
        jLoadMatrix[rowPos(x['P1'],nodes),0:2] += x['peak']*x['normal'][[1,2]]
    for x in jmomnt:
        jLoadMatrix[rowPos(x['P1'],nodes),2] += x['peak']*x['normal'][0]

    memLoadMatrix = zeros((shape(memberLocation)[0],6))
    # jlods = hstack((jforce,jmomnt))
    memloads = loads[apply(lambda x: ((x['degree']>=0 or type(rowPos(x['P1'],nodes))==type(False)) and x['degree']> -3),loads)]
    ...
    for x in memloads:
        memLoadMatrix[dictInArr(x['parentSegment'],segments)] += lineGlobalFEA(x['parentSegment']['P1'],
            x['parentSegment']['P3'],x['P1'],x['P3'],z1(x['parentSegment']),z2(x['parentSegment']),x['degree'],x['peak'],
    ...
    if unique(settlements).any():
        def add(x):
            K = lineStiffnessLocal2(x['youngsModulus'],x['Iyy'],norm(x['P3']-x['P1']),x['area'],x['shearModulus'],shear,inextensible,hinged(x))
            T = lineTransformation2(x['P1'],x['P3'],z1(x),z2(x))
            settlement1 = sum(apply(lambda t: t['settlement'][[1,2,3]],supports[filter1(x)]),0) if filter1(x).any() else zeros(3)
            settlement2 = sum(apply(lambda t: t['settlement'][[1,2,3]],supports[filter2(x)]),0) if filter2(x).any() else zeros(3)
            settlements = hstack((settlement1,settlement2))
            FEAs = T.T @ (K @ (T @ settlements)) if settlements.any() else zeros(6)
            return FEAs
    ...
    for id in arange(segments.size):
        ...
        vec = array([locationMatrix[nj],locationMatrix[nk]]).astype(int).flatten()
        sel = (vec>0).nonzero()[0]
        superStiffness[repeat(vec[sel]-1,sel.size),tile(vec[sel]-1,sel.size)] += lineStiffnessGlobal2(x['P1'],x['P3'],z1(x),z2(x),x['youngsModulus'],x['Iyy'],norm(x['P3']-x['P1']),x['area'],x['shearModulus'],shear,inextensible,hinged(x))[repeat(sel,sel.size),tile(sel,sel.size)]
    ...
    deflections = inv(superStiffness) @ (superJload - superFEA) if superStiffness.size!=0 else 0

    locvec = locationMatrix.flatten().astype(int)

    responses = apply(lambda x: deflections[x-1] if x!=0 else 0, locvec).reshape((shape(nodes)[0],3))
    responses = responses.astype(float)

    a = apply(lambda x: x['settlement'][[1,2,3]], supports)
    responses[spfilter.astype(int)] += a.astype(float)

    responsesRaw = hstack(( nodes,zeros((nodes.shape[0],1)),responses,zeros((nodes.shape[0],2)) ))
    responses = responsesRaw[apply(lambda x: type(rowPos(x,nodesOrg))!=type(False),nodes)][:,[1,2,4,5,6]]
    ...
    for id in arange(segments.size):
        ...
        globalDeflections = apply(lambda x: deflections[x-1] if x!=0 else x, vec)
        memLoad = lineTransformation2(x['P1'],x['P3'],z1(x),z2(x)) @ memLoadMatrix[id] 
        localDeflections = matmul(lineTransformation2(x['P1'],x['P3'],z1(x),z2(x)),globalDeflections)
    ...
    tempLoads = loads[apply(lambda x: x['class'] in ['tempLoad','misfitLoad'],loads)]
    if tempLoads.size>0:
        for x in tempLoads:
            seg = x['parentSegment']['parent'] if x['parentSegment']['parent'] else x['parentSegment']
            actions[2*dictInArr(seg,segmentsOrg) + all(x['P1']==seg['P3']), int(1-2*x['degree'])] -= x['peak']

    actionsRaw = hstack(( actionsRaw[:,0:5],zeros((actionsRaw.shape[0],3)),actionsRaw[:,5].reshape((actionsRaw.shape[0],1)) ))
    actions = hstack(( actions[:,0:5],zeros((actions.shape[0],3)),actions[:,5].reshape((actions.shape[0],1)) ))[:,[1,2,3,4,8]]
    
    # actions = actionsRaw[apply(lambda x: type(rowPos(x[0:3],nodesOrg))!=type(False),actionsRaw)][:,[1,2,3,4,8]]

    multiplier = array([[1,1,1],[-1,-1,-1]])
    globalActions = lambda index: multiplier*(lineTransformation2(segments[index]['P1'],segments[index]['P3'],array([0.0,0.0,1.0]),array([0.0,0.0,1.0])).T @ hstack(actionsRaw[(2*index):(2*index+2),[3,4,8]])).reshape((2,3))
    reactionsRaw = hstack(( actionsRaw[:,:3], vstack(apply(globalActions,arange(segments.size))) ))
    reactions = reactionsRaw[apply(lambda x: type(rowPos(x[:3],nodesOrg))!=type(False),reactionsRaw)]
...
def truss3d(struct,shear=False,inextensible=False,simplified=True):
    ...
    if unique(settlements).any():
        def add(x):
            K = lineStiffnessLocalTruss(x['youngsModulus'],x['area'],norm(x['P3']-x['P1']))
            T = lineTransformationTruss(x['P1'],x['P3'],z1(x),z2(x))
            settlement1 = sum(apply(lambda t: t['settlement'][0:3],supports[filter1(x)]),0) if filter1(x).any() else zeros(3)
            settlement2 = sum(apply(lambda t: t['settlement'][0:3],supports[filter2(x)]),0) if filter2(x).any() else zeros(3)
            settlements = hstack((settlement1,settlement2))
            FEAs = T.T @ (K @ (T @ settlements)) if settlements.any() else zeros(6)
            return FEAs
    ...
    for id in arange(segments.size):
        ...
        vec = array([locationMatrix[nj],locationMatrix[nk]]).astype(int).flatten()
        sel = (vec>0).nonzero()[0]
        superStiffness[repeat(vec[sel]-1,sel.size),tile(vec[sel]-1,sel.size)] += lineStiffnessGlobalTruss(x['P1'],x['P3'],z1(x),z2(x),x['youngsModulus'],x['area'],norm(x['P3']-x['P1']))[repeat(sel,sel.size),tile(sel,sel.size)]
    ...
    for id in arange(segments.size):
        ...
        actions[2*id+1,0:3] = nodes[nk]
        globalDeflections = apply(lambda x: deflections[x-1] if x!=0 else x, vec)
        memLoad = lineTransformationTruss(x['P1'],x['P3'],z1(x),z2(x)) @ memLoadMatrix[id] 
        localDeflections = matmul(lineTransformationTruss(x['P1'],x['P3'],z1(x),z2(x)),globalDeflections)
        localActions = matmul(lineStiffnessLocalTruss(x['youngsModulus'],x['area'],norm(x['P3']-x['P1'])),localDeflections)+memLoad
    ...
    multiplier = array([[1,1,1],[-1,-1,-1]])
    globalActions = lambda index: multiplier*(lineTransformationTruss(segments[index]['P1'],segments[index]['P3'],z1(segments[index]),z2(segments[index])).T @ hstack(actionsRaw[(2*index):(2*index+2),3:])).reshape((2,3))
    reactionsRaw = hstack((actionsRaw[:,:3],vstack(apply(globalActions,arange(segments.size)))))
    reactions = reactionsRaw[apply(lambda x: type(rowPos(x[0:3],nodesOrg))!=type(False),reactionsRaw)]
...
def truss2d(struct,shear=False,inextensible=False,simplified=True,simFac=0.9995):
    ...
    if unique(settlements).any():
        def add(x):
            ...
            settlement1 = sum(apply(lambda t: t['settlement'][1:3],supports[filter1(x)]),0) if filter1(x).any() else zeros(2)
            settlement2 = sum(apply(lambda t: t['settlement'][1:3],supports[filter2(x)]),0) if filter2(x).any() else zeros(2)
            settlements = hstack((settlement1,settlement2))
            FEAs = T.T @ (K @ (T @ settlements)) if settlements.any() else zeros(4)
            return FEAs
    ...
    for id in arange(segments.size):
        ...
        vec = array([locationMatrix[nj],locationMatrix[nk]]).astype(int).flatten()
        sel = (vec>0).nonzero()[0]
        superStiffness[repeat(vec[sel]-1,sel.size),tile(vec[sel]-1,sel.size)] += lineStiffnessGlobalTruss2(x['P1'],x['P3'],z1(x),z2(x),x['youngsModulus'],x['area'],norm(x['P3']-x['P1']))[repeat(sel,sel.size),tile(sel,sel.size)]
    ...
    responses = apply(lambda x: deflections[x-1] if x!=0 else 0, locvec).reshape((shape(nodes)[0],2))
    responses = responses.astype(float)
    ...
    responses = responsesRaw[apply(lambda x: type(rowPos(x,nodesOrg))!=type(False),nodes)][:,[0,1,2,4,5]]
    ...
    for id in arange(segments.size):
        ...
        actionsRaw[2*id+1,0:3] = nodes[nk]
        globalDeflections = apply(lambda x: deflections[x-1] if x!=0 else x, vec)
        memLoad = lineTransformationTruss2(x['P1'],x['P3'],z1(x),z2(x)) @ memLoadMatrix[id] 
        localDeflections = matmul(lineTransformationTruss2(x['P1'],x['P3'],z1(x),z2(x)),globalDeflections)
        localActions = matmul(lineStiffnessLocalTruss(x['youngsModulus'],x['area'],norm(x['P3']-x['P1'])),localDeflections)+memLoad
    ...
    tempLoads = loads[apply(lambda x: x['class'] in ['tempLoad','misfitLoad'] and x['degree']==-1,loads)]
    if tempLoads.size>0:
        for x in tempLoads:
            seg = x['parentSegment']['parent'] if x['parentSegment']['parent'] else x['parentSegment']
            actions[2*dictInArr(seg,segmentsOrg) + all(x['P1']==seg['P3']), 3] -= x['peak']

    multiplier = array([[1,1,1],[-1,-1,-1]])
    globalActions = lambda index: multiplier*(lineTransformationTruss(segments[index]['P1'],segments[index]['P3']).T @ hstack(actionsRaw[(2*index):(2*index+2),3:])).reshape((2,3))
    reactionsRaw = hstack((actionsRaw[:,:3],vstack(apply(globalActions,arange(segments.size)))))
    reactions = reactionsRaw[apply(lambda x: type(rowPos(x[0:3],nodesOrg))!=type(False),reactionsRaw)]
